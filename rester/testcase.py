...
class JsonWrapper(object):
    def __init__(self, d):
        #print "__init__ %s : %s " % (d, type(d))
        if isinstance(d, list):  # if the first level element is a list
            setattr(self, "._length", len(d))
            for index, item in enumerate(d):
                setattr(self, "[%s]" % index, JsonWrapper(item))
        else:            
            for key, value in d.items():
                if isinstance(value, list):
                    setattr(self, key, [JsonWrapper(x) if isinstance(x, dict) else x for x in value])
                    setattr(self, "%s._length" % key, len(value))
                    for index, item in enumerate(value):
                        #setattr(self, "%s[%s]" % (key, index), JsonWrapper(item))
                        setattr(self, "%s[%s]" % (key, index), JsonWrapper(item) if isinstance(item, dict) else item)
                else:
...
class ApiTestCaseRunner:
    ...
    def run_test_suite(self, test_suite_file_name):
        test_suite = self._convert_file_to_json(test_suite_file_name)
        for test_case_file_name in test_suite["test_cases"]:
            self.logger.info("test case : %s", test_case_file_name)
    ...
    def display_report(self):
        ...
        for test_case in self._test_cases:
            ...
            for test_step in test_case.testSteps:
                ...
                print "\n\n     ====> Test Step : {0}, status : {1}".format(test_step.name, "Passed" if test_step.result.status == True else "Failed!")
    ...
    def _convert_file_to_json(self, file):
        with open(file) as file:
            self.logger.info('Processing file %s', file)
            return json.load(file)

    def _process_test_file(self, test_file):
            return JsonWrapper(self._convert_file_to_json(test_file))
    ...
    def _execute_test_case(self, test_case):
        ...
        for test_step in test_case.testSteps:
            ...
            if not skip_all_subsequent_tests:
    ...
    def _execute_test_step(self, test_case, test_step):
        self.logger.debug("inside _execute_test_step %s:%s", test_case.name, test_step.name)
        try:
            method = test_step.method if hasattr(test_step, 'method') else "get"
            self.logger.info(
            ...
            dump_response = True if hasattr(test_step, 'dumpResponse') and test_step.dumpResponse else False
            ...
            json_response_wrapper = self._get_api_json_response_wrapper(
                self._evaluate_expression(test_step.apiUrl), method, headers, params, dump_response)

            if json_response_wrapper.error:
                test_step.result = {"status": False, "message": json_response_wrapper.message}
    ...
    def _assert_element_list(self, test_step, target_response, assert_list):
        self.logger.info("Inside assert_element_list %s ", target_response)
    ...
    def _get_api_json_response_wrapper(self, api_url, method, headers, params, dump_response):
        ...
        if method in allowed_methods:
            ...
            error_msg = 'undefined HTTP method!!! %s', method
            self.logger.error(error_msg)
            return JsonResponseWrapper(json_response.json(), json_response.headers).set_error(error_msg)
        ...
        if json_response.status_code is 200:
            ...
            error_msg = 'API Endpoint Error! Response : %s', json.dumps(
                json_response.json(), sort_keys=True, indent=2)
            self.logger.error(error_msg)
            return JsonResponseWrapper(json_response.json(), json_response.headers).set_error(error_msg)
